...
def read_data(file_path):
    ...
    with open(file_path, "r") as f:
        for line in f:
            match = re.match(r"([\w-]+) X=(\d+), Y=(\d+), Workers=(\d+), Px=(\d+), Py=(\d+), Iter=(\d+), ComputationTime=([\d.]+), TotalTime=([\d.]+), midpoint=([\d.]+)", line)
            if match:
                method, x, y, workers, px, py, iterations, comp_time, total_time, midpoint = match.groups()
                data.append({
                    "Method": method,
                    "X": int(x),
                    "Y": int(y),
...
custom_palette = {"Gauss-Seidel": "#467821", "Jacobi": "#348ABD", "Red-Black": "#A60628"}
...
for x, y in grid_sizes:
    ...
    baseline_times = subset[subset["Workers"] == 1].set_index("Method")["TotalTime"].to_dict()
    subset["Speedup"] = subset.apply(lambda row: baseline_times[row["Method"]] / row["TotalTime"], axis=1)
    ...
    plt.xticks(sorted(subset["Workers"].unique()), labels=sorted(subset["Workers"].unique()), fontsize=12)
