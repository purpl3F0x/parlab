            ...
            double* clusters)       /* out: [numClusters][numCoords] */
{
    ...
    int *  rank_newClusterSize,
      *newClusterSize; // [numClusters]: no. objects assigned in each new cluster
    ...
    rank_newClusterSize =
      (typeof(rank_newClusterSize))calloc(numClusters, sizeof(*rank_newClusterSize));
    rank_newClusters =
      (typeof(rank_newClusters))calloc(numClusters * numCoords, sizeof(*rank_newClusters));
    ...
    do {
        ...
        MPI_Allreduce(rank_newClusters,
                      newClusters,
        ...
        MPI_Allreduce(
          rank_newClusterSize, newClusterSize, numClusters, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        fprintf(stdout,
                "nodes = %2d, nloops = %3d, total = %7.4fs, per loop = %7.4fs\n",
                nodes,
                loop,
                timing,
                timing / loop);
