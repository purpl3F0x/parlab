...
fw_seq = pd.read_csv(os.path.join(METRICS_FOLDER, "fw_seq.csv")).rename(columns=lambda x: x.strip())
...
for batch_size in fw_sr["BATCH_SIZE"].unique():
    plt.figure(figsize=(14, 7))
    subset = fw_sr[fw_sr["BATCH_SIZE"] == batch_size].copy()  # Use copy to avoid the warning
    ...
    seq_data = pd.DataFrame({
        ...
        "TIME": [seq_time],
        "TILE_SIZE": ["None"]  # Set a string placeholder for sequential's TILE_SIZE
    })
...
for batch_size in fw_tiled["BATCH_SIZE"].unique():
    plt.figure(figsize=(14, 7))
    subset = fw_tiled[fw_tiled["BATCH_SIZE"] == batch_size].copy()  # Use copy to avoid the warning
