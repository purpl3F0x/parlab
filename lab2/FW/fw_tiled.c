...
void FW_SSE(int** A, int K, int I, int J, int N) {
    for (int k = K; k < K + N; k++) {
        for (int i = I; i < I + N; i++) {
            // Broadcast A[i][k] to all elements of an SSE register as void*s
            __m128i a_ik = _mm_set1_epi32(A[i][k]);

            int j = J;
            for (; j <= J + N - 16; j += 16) { // Unroll by 4 (4 * 4 = 16 elements)
                // Load A[i][j:j+15] and A[k][j:j+15] as ints
                __m128i a_ij0 = _mm_loadu_si32((void*)&A[i][j]);
                __m128i a_ij1 = _mm_loadu_si32((void*)&A[i][j + 4]);
                __m128i a_ij2 = _mm_loadu_si32((void*)&A[i][j + 8]);
                __m128i a_ij3 = _mm_loadu_si32((void*)&A[i][j + 12]);

                __m128i a_kj0 = _mm_loadu_si32((void*)&A[k][j]);
                __m128i a_kj1 = _mm_loadu_si32((void*)&A[k][j + 4]);
                __m128i a_kj2 = _mm_loadu_si32((void*)&A[k][j + 8]);
                __m128i a_kj3 = _mm_loadu_si32((void*)&A[k][j + 12]);

                // Compute A[i][k] + A[k][j:j+15] for each block
                ...
                // Store the results back to A[i][j:j+15] as void*s
                _mm_storeu_si32(&A[i][j], min_val0);
                _mm_storeu_si32(&A[i][j + 4], min_val1);
                _mm_storeu_si32(&A[i][j + 8], min_val2);
                _mm_storeu_si32(&A[i][j + 12], min_val3);
            }
