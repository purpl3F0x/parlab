...
__host__ __device__ inline static double euclid_dist_2(int     numCoords,
    ...
    for (i = 0; i < numCoords; i++) {
        auto diff = objects[objectId * numCoords + i] - clusters[clusterId * numCoords + i];
        ans += diff * diff;
    }
...
void kmeans_gpu(double* objects,        /* in: [numObjs][numCoords] */
    ...
    const unsigned int numClusterBlocks =
      (numObjs + numThreadsPerClusterBlock - 1) /
    ...
    checkCuda(cudaMemcpy(
      deviceObjects, objects, numObjs * numCoords * sizeof(double), cudaMemcpyHostToDevice));
    checkCuda(
    ...
    do {
        ...
        for (i = 0; i < numClusters; i++) {
            for (j = 0; j < numCoords; j++) {
                ...
                newClusters[i][j] = 0; /* set back to 0 */
            }
