...
__global__ static void find_nearest_cluster(int     numCoords,
    ...
    if (tid < numObjs) {
        ...
        index = 0;
        // TODO: call min_dist = euclid_dist_2_transpose(...) with correct objectId/clusterId
        min_dist =
          euclid_dist_2_transpose(numCoords, numObjs, numClusters, objects, deviceClusters, tid, 0);

        for (i = 1; i < numClusters; i++) {
            // TODO: call dist = euclid_dist_2_transpose(...) with correct objectId/clusterId
            dist = euclid_dist_2_transpose(
              numCoords, numObjs, numClusters, objects, deviceClusters, tid, i);
...
void kmeans_gpu(double* objects,        /* in: [numObjs][numCoords] */
    ...
    printf("\n|-----------Transpose GPU K-means------------|\n\n");

    // TODO: Copy objects given in [numObjs][numCoords] layout to new [numCoords][numObjs] layout
    for (i = 0; i < numObjs; i++) {
    ...
    do {
        ...
        // TODO: Copy clusters to deviceClusters checkCuda(cudaMemcpy(...));
        ...
        // TODO: Copy deviceMembership to membership checkCuda(cudaMemcpy(...));
        checkCuda(
          cudaMemcpy(membership, deviceMembership, numObjs * sizeof(int), cudaMemcpyDeviceToHost));

        // TODO: Copy dev_delta_ptr to &delta checkCuda(cudaMemcpy(...));
